...
class TestWorkspaceIsCopiedNotMounted:
    """Isolation per run: nothing of the host is reachable from inside.

    The workspace is copied in and copied back out rather than bind-mounted.
    A bind mount's *source* is resolved on the host, so a symlinked source
    exposed whatever it pointed at — and a repository decides what it stores at
    a path, so validating the config could never prevent it. Copying makes the
    escape impossible instead of checked-for.
    """

    @pytest.fixture
    @staticmethod
    def runner(docker: DockerCli):
        from jormungandr.runtime.run import PromptRunner
    ...
    @pytest.fixture(scope="class")
    @staticmethod
    def image(docker: DockerCli, tmp_path_factory):
        """An image shaped the way compile_image_spec builds them.

        With the user and workdir modules, so the agent is unprivileged and
        /workspace exists — which is what makes copied-in ownership matter.
        """
        from jormungandr.runtime.build import ImageBuilder
        from jormungandr.runtime.compose import compose
        from jormungandr.runtime.spec import ImageSpec
        ...
        spec = ImageSpec(
            base_image="alpine:3.20",
            repository="jormungandr-wscopy",
            modules=[{"name": "user"}, {"name": "workdir"}],
        )
        composed = compose(spec)
        builder = ImageBuilder(state_dir=tmp_path_factory.mktemp("ws"), docker=docker)
        result = builder.build(spec)
        yield result.reference
        for layer in reversed(composed.layers):
            docker.remove_image(layer.reference, force=True)

    def test_content_arrives_and_changes_come_back(
        self, runner, image, tmp_path
    ) -> None:
        workspace = tmp_path / "ws"
        workspace.mkdir()
        (workspace / "input.txt").write_text("from the host\n")

        run = runner.run(
            harness="droid",
            image=image,
            prompts=["ignored by the stub"],
            workspace=workspace,
            workdir="/workspace",
            invocation=_EchoInvocation(),
        )
        assert run.ok, run.turns[0].stderr
        # the agent saw the input...
        assert "from the host" in run.turns[0].stdout
        # ...and its own edit came back out
        assert (workspace / "made-by-agent.txt").read_text().strip() == "hello"

    def test_a_symlink_to_a_host_path_is_inert_inside(
        self, runner, image, tmp_path
    ) -> None:
        ...
        run = runner.run(
            ...
            workspace=workspace,
            workdir="/workspace",
            invocation=_ReadEscapeInvocation(),
        )
        # The symlink resolves inside the container, where that path does not
        # exist, so the host file is unreachable.
        assert "HOST SECRET" not in run.turns[0].stdout
        assert secret.read_text() == "HOST SECRET"


class _EchoInvocation:
    harness = "droid"
    state_paths = ()
    system_via = "argv"
    ...
    def build(self, prompt, *, model=None, system=None):
        ...
        return Invocation(
            argv=(
                "sh",
                "-c",
                "cat /workspace/input.txt; echo hello > /workspace/made-by-agent.txt",
            )
        )


class _ReadEscapeInvocation:
    harness = "droid"
    state_paths = ()
    system_via = "argv"
